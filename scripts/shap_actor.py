...
for i in range(args.procs):
    env = utils.make_env(args.env, args.seed + 10000 * i, args.top_down)
    # breakpoint()
    envs.append(env)
...
class ActorModel(nn.Module):
    def __init__(self, agent):
        ...
        self.actor = agent.acmodel.actor
...
# e = shap.DeepExplainer(model, data[numpy.random.choice(len(background), int(0.8*len(background)), replace=False)])
...
shap_val = [numpy.transpose(shap_values[i], (0,3,2,1)) for i in range(7)]
shap_val = [numpy.array([scale(shap_val[i][j], 112, 112) for j in range(5)]) for i in range(7)]
# shap_val = numpy.array(shap_val)

breakpoint()
shap.image_plot(shap_val, numpy.array(images)/255.) #,labels=[action_names]+[dummy_names]*4)
filename = f'{args.savedir}/shap_plot_actor-{args.env}_model-{args.model}_{args.tag}'
print(filename)
plt.savefig(filename +'.png') # f'{args.savedir}/shap_plot_critic_env-{args.env}_model-{args.model}')


x = action_probs
print('action_probs:\n', numpy.exp(x)/numpy.exp(x).sum())
